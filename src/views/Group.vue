...
export default {
  ...
  computed: {
    ...mapState(['updating', 'loading', 'tasks']),
    ...mapGetters(['groupItems'])
  },
  methods: {
    ...mapActions(['fetchGroups', 'addGroup', 'deleteGroup']),
    deleteClick(group) {
    ...
    deleteDialogConfirmclick() {
      ...
      const taskInGroup = this.tasks.find(task => task.group_id === this.currentGroup.value)
      if (taskInGroup) {
        this.$store.commit('SHOW_SNACKBAR', 'There are tasks under this group, you cannot delete it now!')
      } else {
        this.deleteGroup(this.currentGroup.value)
      }
    },
    addGroupClick() {
